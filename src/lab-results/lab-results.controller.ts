...
export class LabResultsController {
  ...
  async update(
    ...
    const doctorInChargeForExamination =
      await this.labResultsService.findDoctorByLabResultId(id);
    if (req.user.email != doctorInChargeForExamination.doctorEmail) {
      throw new HttpException(
        'You are not allowed to change lab result details.',
        HttpStatus.FORBIDDEN,
      );
    }
  ...
  async remove(@Req() req, @Param('id') id: number) {
    ...
    if (req.user.email != doctorInChargeForExamination.doctorEmail) {
      throw new HttpException(
        'You are not allowed to delete lab result.',
        HttpStatus.FORBIDDEN,
      );
    }
